...
public class FamilyValidatorTest extends ValidatorLoader {
    ...
    //US17
    @Test
    public void parentsShouldNotMarryDescendents() {
    	//arrange
    	GedLogger logger = mock(GedLogger.class);
    	Load("test_us17", logger);
    	//act
    	boolean result = fv.parentsAndDescendantsShouldNotMarry();
    	//assert
    	verify(logger, Mockito.times(1)).error("US17", repository.GetIndividual("US17_IID3"), repository.GetFamily("US17_FID2"), "Child cannot be married to parent");
    	verifyNoMoreInteractions(logger);
    	Assert.assertFalse(result);
    }
    ...
    //US31
    @Test
    public void listLivingNeverMarriedTest() {
    	//arrange
        GedLogger logger = mock(GedLogger.class);
        Load("test_us31", logger);
        //act
        fv.listLivingNeverMarried();
        //assert
        verify(logger, Mockito.times(1)).info("US31", repository.GetIndividual("US31_IID3"), null, "Individual was never married.");
        verify(logger, Mockito.times(1)).info("US31", repository.GetIndividual("US31_IID4"), null, "Individual was never married.");
        verifyNoMoreInteractions(logger);
    }
}
